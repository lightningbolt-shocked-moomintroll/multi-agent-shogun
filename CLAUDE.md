...
1. **自分の役割を確認**（以下のいずれかの方法）:
   - **【推奨】環境変数で確認**: `echo $AGENT_ID`
     - 出力例: `karo`, `ashigaru1`, `ashigaru2`, ..., `ashigaru8`
   - **ウィンドウ名で確認**: `tmux display-message -p '#W'`
   - **ウィンドウインデックスで確認**: `tmux display-message -p '#{window_index}'`
     - `0` → karo（家老）
     - `1` → ashigaru1（足軽1）
     - `2` → ashigaru2（足軽2）
     - ...
...
2. **対応する instructions を読む**:
   ...
   - karo (multiagent:0) → instructions/karo.md
   - ashigaru (multiagent:1-8) → instructions/ashigaru.md
3. **禁止事項を確認してから作業開始**
...
### multiagentセッション（9ウィンドウ）
- ウィンドウ0: karo（家老）
- ウィンドウ1-8: ashigaru1-8（足軽）

### 操作方法
- `Ctrl+b n` - 次のウィンドウ
- `Ctrl+b p` - 前のウィンドウ
- `Ctrl+b 0-8` - ウィンドウ番号で直接移動
- `Ctrl+b w` - ウィンドウ一覧メニュー
...
### 4. 家老の状態確認
- 指示前に家老が処理中か確認: `tmux capture-pane -t multiagent:0 -p | tail -20`
