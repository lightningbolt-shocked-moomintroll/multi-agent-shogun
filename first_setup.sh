...
# ============================================================
# STEP 5: jq チェック（権限管理に必要）
# ============================================================
log_step "STEP 5: jq チェック"

if command -v jq &> /dev/null; then
    JQ_VERSION=$(jq --version)
    log_success "jq がインストール済みです ($JQ_VERSION)"
    RESULTS+=("jq: OK ($JQ_VERSION)")
else
    log_warn "jq がインストールされていません（権限管理に必要）"
    echo ""

    if command -v apt-get &> /dev/null; then
        read -p "  jq をインストールしますか? [Y/n]: " REPLY
        REPLY=${REPLY:-Y}
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            if command -v jq &> /dev/null; then
                log_success "jq インストール完了"
                RESULTS+=("jq: インストール完了")
            else
                log_error "jq のインストールに失敗しました"
                RESULTS+=("jq: インストール失敗")
                HAS_ERROR=true
            fi
        else
            log_warn "jq のインストールをスキップしました"
            RESULTS+=("jq: 未インストール (スキップ)")
        fi
    else
        log_error "apt-get が見つかりません。手動で jq をインストールしてください"
        echo "  Ubuntu/Debian: sudo apt-get install jq"
        echo "  macOS:         brew install jq"
        RESULTS+=("jq: 未インストール (手動インストール必要)")
    fi
fi
...
SETTINGS_EOF
    log_success "権限設定ファイルを作成しました"
    log_info "  └─ 設定変更: ./scripts/manage_permissions.sh"
else
    log_info "権限設定ファイルは既に存在します"
fi
...
SCRIPTS=(
    ...
    "scripts/safe_send_keys.sh"
    "scripts/sanitize_input.sh"
    "scripts/validate_path.sh"
)
